...
echo "2/4  painting out the timestamp and scale bar"
python3 scripts/deburn-overlays.py "$WORK/raw" "$WORK/clean"

echo "3/4  levelling ${ANGLE} deg, cropping to ${CROP}, ping-pong loop"
R="${ANGLE}*PI/180"
...
ffmpeg -v error -framerate 8 -i "$WORK/clean/f%03d.png" \
